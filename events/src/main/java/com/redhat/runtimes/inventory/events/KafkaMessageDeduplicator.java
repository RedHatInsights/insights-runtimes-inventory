...
public class KafkaMessageDeduplicator {
  ...
  /**
   * Extracts the message ID from a Kafka message header value. If multiple header values are
   * available, the first one will be used and the other ones will be ignored. An invalid header
   * value will be counted and logged, but won't interrupt the message processing: the deduplication
   * will be disabled for the message.
   */
  public UUID findMessageId(String bundleName, String applicationName, Message<String> message) {
      ...
      if (headers.hasNext()) {
        ...
        if (header.value() == null) {
          ...
          try {
            ...
            Log.tracef(
                "Application %s/%s sent a valid Kafka header [%s=%s]",
    ...
    if (!found) {
      ...
      Log.tracef(
          "Application %s/%s did not send any Kafka header [%s]",
          bundleName, applicationName, MESSAGE_ID_HEADER);
    }
  ...
  /**
   * Verifies that another Kafka consumer didn't already process the given message and then failed
   * to commit its offset. Such failure can happen when a consumer is kicked out of its consumer
   * group because it didn't poll new messages fast enough. We experienced that already on
   * production.
   */
  ...
  //            kafkaMessage.prePersist(); // This method must be called manually while using a
  // StatelessSession.
